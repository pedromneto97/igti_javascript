...
function findBestPetShop(smallAnimalsCount, bigAnimalsCount, isWeekend) {
    let cheaper;

    for (let i = 0; i < petshops.length; i++) {
        const { name, distance, bath } = petshops[i];
        const price = getPetShopPrice(smallAnimalsCount, bigAnimalsCount, isWeekend, bath, false);
...
module.exports = {
    findBestPetShopPart1: findBestPetShop,
};
